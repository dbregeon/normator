...
class ResolutionGraph(val nodes : Set[Source[_]]) {
  val arcs : Map[Source[_], Source[_]] = {
    val propertyMap : Map[Property, Source[_]] = nodes.map(node => (node.outputProperty, node)).toMap

    nodes.foldLeft(Map.empty[Source[_], Source[_]])((fullMap, node) => {
      node.inputProperties.foldLeft(fullMap)((map, p) => {
        propertyMap.get(p).foldLeft(map)((m, endNode) => m + ((node, endNode)))
      })
    })
  }

  val inputs : Set[Property] = nodes.diff(arcs.keySet).flatMap(n => n.inputProperties)
